...
- Node.js 22, Git, Bash, and network access only to GitHub, internal 9Router, Infisical for publishing, and approved package registries.
...
- A GitHub App installed on the pilot repository with Metadata read, Contents read/write, Issues read/write, and Pull requests read/write. Set its App ID as `AI_GITHUB_APP_ID`.
- An Infisical machine identity trusting GitHub Actions OIDC. Set `AI_INFISICAL_IDENTITY_ID`, `AI_INFISICAL_PROJECT_ID`, `AI_INFISICAL_ENVIRONMENT`, and the actual App-private-key secret name in `AI_INFISICAL_APP_PRIVATE_KEY_SECRET`. Optional overrides are `AI_INFISICAL_BASE_URL`, `AI_INFISICAL_OIDC_AUDIENCE`, and `AI_INFISICAL_SECRET_PATH`.
- The consumer allows reusable workflows from `caesariodito/.standardization` and defines the labels below.

The Infisical OIDC and 9Router API contracts must be smoke-tested against the platform-owned endpoints before a real issue. No endpoint, identity, runner label, or App ID is supplied by this repository.
...
2. Configure the variables and secret above. Optionally set `AI_PLANNER_MODEL`, `AI_PLANNER_TIMEOUT_MINUTES`, `AI_IMPLEMENTER_MODEL`, `AI_IMPLEMENTER_TIMEOUT_MINUTES`, and trusted `AI_IMPLEMENTER_CHECK_COMMAND`.
...
The Planner has only `contents: read`, `issues: write`, and Pi's read tools. The model-running Implementer job has no App token or OIDC permission. A second ephemeral publishing job retrieves the App key through OIDC only after useful work is bundled, mints a repository-scoped short-lived token, pushes, opens a draft PR, and revokes the token.
